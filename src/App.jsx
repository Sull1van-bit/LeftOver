...
// --- INITIAL MOCK DATA (Translated to English) ---
...
  allProducts: [
    { id: 1, name: "Chocolate Croissant", images: ['https://placehold.co/600x600/EFE3C2/123524?text=Bread%201', 'https://placehold.co/600x600/EFE3C2/123524?text=Bread%202', 'https://placehold.co/600x600/EFE3C2/123524?text=Bread%203', 'https://placehold.co/600x600/EFE3C2/123524?text=Bread%204', 'https://placehold.co/600x600/EFE3C2/123524?text=Bread%205'], price: 112500, discountPrice: null, stock: 5, sales: 15, status: 'Live', desc: 'Crispy croissant with a premium melted chocolate filling.', views: 350, conversion: 0.042 },
...
    { id: 3, name: "Sugar Donut", images: ['https://placehold.co/600x600/EFE3C2/123524?text=Cake', 'https://placehold.co/600x600/EFE3C2/123524?text=Cake%202', 'https://placehold.co/600x600/EFE3C2/123524?text=Cake%203', 'https://placehold.co/600x600/EFE3C2/123524?text=Cake%204', 'https://placehold.co/600x600/EFE3C2/123524?text=Cake%205'], price: 75000, discountPrice: null, stock: 10, sales: 30, status: 'Live', desc: 'Soft donut with a sweet powdered sugar topping.', views: 820, conversion: 0.036 },
...
  ],
  promotions: [ { id: 1, name: "Flash Bread Discount", type: "Product Discount", status: "Ongoing", period: "2025-07-24 - 2025-07-25", desc: "Get a 20% discount on all bread products. Perfect for breakfast or an afternoon snack!", performance: { products_sold: 150, revenue: 18750000 } }, { id: 2, name: "Cake Price Cut", type: "Store Voucher", status: "Upcoming", period: "2025-07-26 - 2025-07-28", desc: "Voucher potongan harga Rp 15.000 untuk semua jenis kue bolu dan tart. Pembelian minimum Rp 75.000.", performance: null}, { id: 3, name: "Payday Promo", type: "Product Discount", status: "Finished", period: "2025-06-25 - 2025-06-26", desc: "Enjoy special prices on our best-selling products during the payday period.", performance: { products_sold: 320, revenue: 67500000 } }, ],
  dataCompass: {
    kpi: [
...
                {chartData.map(item => <span key={item.day} className="text-xs text-gray-400 flex-1 text-center">{item.day}</span>)}
            </div>
        </div>
    );
};
...
// --- Page Views ---
const HomepageView = ({ data, onAddProductClick }) => {
...
                    <button className="bg-[#85A947] text-[#123524] font-bold p-4 rounded-lg flex flex-col items-center justify-center hover:bg-[#a0c45e] transition-all duration-300 transform hover:-translate-y-1"><Icon path={ICONS.tag} className="w-8 h-8 mb-2"/><span className="font-semibold text-sm text-center">Create Promotion</span></button>
                </div>
            </div>
...
                 <h2 className="text-xl font-bold text-[#123524]">Sales Analysis</h2>
                 <p className="text-sm text-gray-500 mb-4">Sales performance for the last 7 days.</p>
                 <LineChart data={data.summary.dailyChart} color="#3E7B27" dataKey="sales" labelKey="day" />
            </div>
        </div>
    );
}
...
const MarketingView = ({ data, onDetailClick }) => {
    const [activeTab, setActiveTab] = useState('All');
...
    return (
        <div className="p-6"><div className="flex justify-between items-center mb-4"><h1 className="text-2xl font-bold text-gray-800">Marketing Center</h1><button className="bg-[#85A947] text-[#123524] font-bold px-4 py-2 rounded-md hover:bg-[#a0c45e] transition-colors flex items-center gap-2"><Icon path={ICONS.plus} className="w-5 h-5" />Create Promotion</button></div><Card className="p-0">
...
            <div className="overflow-x-auto"><table className="w-full text-sm text-left text-gray-500"><thead className="text-xs text-gray-700 uppercase bg-gray-50"><tr><th scope="col" className="px-6 py-3">Promotion Name</th><th scope="col" className="px-6 py-3">Type</th><th scope="col" className="px-6 py-3">Period</th><th scope="col" className="px-6 py-3">Status</th><th scope="col" className="px-6 py-3 text-center">Action</th></tr></thead>
                <tbody>{filteredPromotions.map(promo => (<tr key={promo.id} className="bg-white border-b hover:bg-gray-50"><td className="px-6 py-4 font-medium text-gray-900">{promo.name}</td><td className="px-6 py-4">{promo.type}</td><td className="px-6 py-4">{promo.period}</td><td className="px-6 py-4"><StatusBadge status={promo.status} /></td><td className="px-6 py-4 text-center"><button onClick={() => onDetailClick(promo)} className="font-medium text-[#3E7B27] hover:underline transition-transform duration-150 active:scale-95">View</button></td></tr>))}</tbody>
            </table></div></Card></div>
    );
}
...
        setCompassData({
            overviewChart: newData.overviewChart,
            trafficChart: newData.trafficChart,
        });
    }, [dateRange]);
...
                const sortedBySales = [...allProducts].sort((a,b) => b.sales - a.sales);
...
                                <p className="text-sm text-gray-500 mb-4">Upload a minimum of {MIN_IMAGES} photos to list the product.</p>
                                <div className="space-y-4">
                                    {images.length === 0 ? (
...
                            Please complete all fields and upload at least {MIN_IMAGES} photos.
                        </p>
                    )}
...
                            <p className="font-medium text-gray-800">{promo.period}</p>
                        </div>
                        <div>
...
// --- Layout Components ---
const Sidebar = ({ activeMenu, setActiveMenu, activeSubMenu, setActiveSubMenu, hasNewOrder }) => {
...
                <a href="#" className="flex items-center justify-center lg:justify-start p-3 rounded-lg hover:bg-red-500/50"><Icon path={ICONS.logout} className="h-6 w-6 text-white" /><span className="hidden lg:block ml-4 font-semibold text-white">Logout</span></a>
            </div>
        </div>
    );
};

const DashboardLayout = ({ data, setData, setView, setEditingProduct }) => {
...
        switch (view) {
            case 'Homepage': return <HomepageView data={data} onAddProductClick={() => setView('addProduct')} />;
...
            />;
            case 'Marketing': return <MarketingView data={data} onDetailClick={setPromoToDetail} />;
...
            <Sidebar activeMenu={activeMenu} setActiveMenu={setActiveMenu} activeSubMenu={activeSubMenu} setActiveSubMenu={setActiveSubMenu} hasNewOrder={hasNewOrder} />
...
    const [view, setView] = useState('login'); // 'login', 'register', 'dashboard', 'addProduct', 'editProduct'
...
            case 'dashboard': return <DashboardLayout data={data} setData={setData} setView={setView} setEditingProduct={setEditingProduct} />;
